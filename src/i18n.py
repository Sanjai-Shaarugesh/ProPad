...
Enhanced internationalization support for ProPad with automatic language detection
and comprehensive string translation support
"""
...
def _get_locale_dir():
    """Determine the locale directory based on installation type."""
    # Priority order for locale directory detection
    possible_dirs = [
        "/app/share/locale",  # Flatpak installation
        "/usr/share/locale",  # System-wide installation
        "/usr/local/share/locale",  # Local system installation
    ]

    # Check standard system locations first
    for dir_path in possible_dirs:
        locale_path = os.path.join(dir_path, APP_NAME, "LC_MESSAGES")
        if os.path.exists(locale_path):
            return dir_path

    # Development mode - check project directory
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    dev_locale = os.path.join(base_dir, "locale")
    if os.path.exists(dev_locale):
        return dev_locale

    # Fallback to development directory even if it doesn't exist yet
    return dev_locale


# Initialize locale directory as a constant
LOCALE_DIR = _get_locale_dir()

# Global translation function (will be set by init_locale)
_ = lambda s: s  # Default fallback


def _detect_system_language():
    """Detect the system language from various sources."""
    detected_lang = None
    ...
    for env_var in ["LANGUAGE", "LC_ALL", "LC_MESSAGES", "LANG"]:
        ...
        if lang:
            # Extract language code (e.g., 'en_US.UTF-8' -> 'en_US' or 'en')
            lang_clean = lang.split(".")[0].split(":")[0]

            # Check if it's a format like zh_CN, ta_IN etc
            if "_" in lang_clean:
                detected_lang = lang_clean  # Keep full locale (zh_CN)
            else:
                detected_lang = lang_clean.split("_")[0]  # Just language (en)

            if detected_lang and len(detected_lang) >= 2:
                print(f"🌍 Detected language from {env_var}: {detected_lang}")
                break

    # Method 2: Use locale.getdefaultlocale()
...
def init_locale():
    ...
    # Try finding any variant (zh -> zh_CN)
    else:
    ...
    try:
        ...
        # Get translation for detected language
        if lang_to_use != "en":
            try:
...
LANGUAGE_NAMES = {
    ...
    "ar": "العربية",
    "hi": "हिन्दी",
    "bn": "বাংলা",  # Bengali (already present)
    "gu": "ગુજરાતી",  # ← Added Gujarati
    "ta": "தமிழ்",
    "te": "తెలుగు",
    "mr": "मराठी",
    "tr": "Türkçe",

...
class TableGridSelector(Gtk.Popover):
    ...
    def _load_css_from_file(self):
        """Load CSS from separate file or fallback to inline"""
...
# Modified FormattingToolbar class
...
class FormattingToolbar(Gtk.Popover):
    ...
    def _insert_image(self, button):
        """
        Use Gtk.FileChooserDialog instead of Gtk.FileDialog.
        This forces the use of the native/local file chooser which returns
        absolute filesystem paths (e.g., /home/user/...) instead of
        Portal paths (e.g., /run/user/...)
        """
        dialog = Gtk.FileChooserDialog(
            title=_("Select Image"),
            transient_for=self.parent_window,  # Correct property for parent window
        ...
        # Set initial folder to Pictures directory
        try:
    ...
    def _on_image_dialog_response(self, dialog, response):
        ...
        if response == Gtk.ResponseType.ACCEPT:
            ...
            if file:
                # Get the absolute path (e.g., /home/sanjai/Pictures/...)
                filepath = file.get_path()
                filename = file.get_basename()

                # Insert the absolute path directly
                cursor = self.buffer.get_iter_at_mark(self.buffer.get_insert())
